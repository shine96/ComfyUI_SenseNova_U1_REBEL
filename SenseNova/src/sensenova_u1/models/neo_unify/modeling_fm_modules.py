...
class TimestepEmbedder(nn.Module):
    ...
    def forward(self, t):
        ...
        # GGUF-safe dtype check
        mlp_first = self.mlp[0]
        if hasattr(mlp_first, "weight") and mlp_first.weight is not None:
            target_dtype = mlp_first.weight.dtype
        else:
            target_dtype = getattr(mlp_first, "dtype", torch.bfloat16)
...
class FlowMatchingHead(nn.Module):
    ...
    def dtype(self):
        # GGUF-safe dtype check
        if hasattr(self.net.input_proj, "weight") and self.net.input_proj.weight is not None:
            return self.net.input_proj.weight.dtype
        return getattr(self.net.input_proj, "dtype", torch.bfloat16)

    @property
    def device(self):
        # GGUF-safe device check
        if hasattr(self.net.input_proj, "weight") and self.net.input_proj.weight is not None:

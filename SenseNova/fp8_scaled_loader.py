...
class FP8ScaledLinear(_ConvRotBase):
    ...
    def forward(self, x):
        ...
        # FP8 tensor-core fast path needs a weight scale; skip it for --scale-mode none.
        if self.use_scaled_mm and self.has_scale and x.is_cuda and w.dtype.is_floating_point:
            try:
                # FP8 tensor-core fast path (mirrors ComfyUI native). Inputs must be fp8.
                x_fp8 = x.to(torch.float8_e4m3fn)
                w_fp8 = self.weight_fp8.to(x_fp8.device)
                # per-tensor input scale + per-output-channel weight scale
                in_scale = x_fp8.abs().amax().clamp(min=1e-8) / 448.0
                out, _ = torch._scaled_mm(
                    x_fp8, w_fp8.T,
                    scale_a=in_scale.reshape(1, 1),
                    scale_b=self.weight_scale.reshape(-1, 1).to(x_fp8.device),
                ...
                pass  # fall through to the de-quant F.linear path below
